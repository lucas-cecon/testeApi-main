        ...
        <div class="container mx-auto py-6 w-11/12">
            <div class="shadow-md rounded-lg overflow-hidden">
                <div class="overflow-x-auto bg-gray-100"> <!-- Fundo alterado para a mesma cor do resto da página -->
                    ...
                    @else
                        <!-- Formulário de busca -->
                        <form method="GET" action="{{ route('dashboard.rh.pesquisar') }}" id="searchForm" class="flex flex-col md:flex-row items-center justify-center mb-6">
                            <input 
                                type="text" 
                                class="block w-full md:w-1/3 p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500"
                                id="searchInput" 
                                name="search" 
                                placeholder="Buscar por funcionário ou status" 
                                ...
                                @foreach($tickets as $ticket)
                                    <tr class="border-b">
                                        ...
                                        <td class="border border-gray-300 px-4 py-2">
                                            ...
                                            ({{ $ticket->horarioAntigo->hora_inicio }} - {{ $ticket->horarioAntigo->hora_fim }})
                                        </td>
                                        <td class="border border-gray-300 px-4 py-2">
                                            {{ $ticket->horarioNovo->codigo }} 
                                            ({{ $ticket->horarioNovo->hora_inicio }} - {{ $ticket->horarioNovo->hora_fim }})
                                        </td>
                                        <td class="border border-gray-300 px-4 py-2">{{ $ticket->statusTicket->status }}</td>
                                        <td class="border border-gray-300 px-4 py-2">
                                            <a href="{{ route('dashboard.rh.show_ticket', $ticket->id) }}" class="text-blue-600 hover:underline">
                                                Visualizar
                                            </a>
                                        </td>
                                    </tr>

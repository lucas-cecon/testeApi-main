        ...
        <div class="bg-gray-200 p-8 rounded-md shadow-md w-full max-w-2xl"> <!-- Tornado responsivo com w-full -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" id="funcionario" value="{{ $diploma->id }}"
                        class="bg-gray-200 border border-red-500 p-2 rounded-md w-full" readonly>
                </div>

                <!-- Horário Antigo -->
                <div class="flex flex-col">
                    <label for="horario-antigo" class="font-bold mb-2">Título:</label>
                    <input type="text" id="horario-antigo" value="{{ $diploma->titulo }}"
                        class="bg-gray-200 border border-red-500 p-2 rounded-md w-full" readonly>
                </div>

                <div class="flex flex-col">
                    <label for="horario-antigo" class="font-bold mb-2">Lote do diploma:</label>
                    <input type="text" id="horario-antigo" value="{{ $diploma->lote_diploma }}"
                        class="bg-gray-200 border border-red-500 p-2 rounded-md w-full" readonly>
                </div>

                <div class="flex flex-col">
                    <label for="horario-antigo" class="font-bold mb-2">Quantidade:</label>
                    <input type="text" id="horario-antigo" value="{{ $diploma->quant_diploma }}"
                        class="bg-gray-200 border border-red-500 p-2 rounded-md w-full" readonly>
                </div>

                <div class="flex flex-col">
                    <label for="horario-antigo" class="font-bold mb-2">Turma:</label>
                    <input type="text" id="horario-antigo"
                        value="{{ $diploma->curso ? $diploma->curso->curso : 'Turma não encontrada' }}"
                        class="bg-gray-200 border border-red-500 p-2 rounded-md w-full" readonly>
                </div>
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" id="horario-antigo"
                        value=" @if ($diploma->status == 1) Aberto
...
Desconhecido @endif"
                ...
                <div class="flex flex-col grid-cols-">
                    @if ($diploma->status < 3)
                        <form method="POST" action="{{ route('diplomas.atualizarStatus', $diploma->id) }}"
                            style="display:inline;" onsubmit="return confirmUpdate();">
                            @csrf
                ...
                <div class="flex flex-col grid-cols-">
                    ...
                    <form id="filterForm" class="flex flex-col md:flex-row items-center justify-center mb-6">
                        <input type="text" name="search"
                            ...
                            placeholder="Buscar por Nome do Aluno ou CPF" value="{{ request('search') }}" />
                    ...
                    <div>
                        <button type="button" id="addAlunoButton"
                            class="btn btn-primary w-full border bg-red-500 hover:bg-red-700 text-white border-red-500 p-2 rounded-md">
                            Adicionar Aluno
                        </button>
                    </div>
                    </form>
                        ...
                        <table class=" table-auto w-full border-collapse border border-gray-200">
                            ...
                            <tbody class="text-gray-700 text-sm">
                                @foreach ($alunosRelacionados as $relacao)
                                    <tr class="border-b">
                                        <td class="border border-gray-300 px-4 py-2">{{ $relacao->aluno->id_aluno }}
                                        </td>
                                        <td class="border border-gray-300 px-4 py-2">{{ $relacao->aluno->nome }}</td>
                                        <td class="border border-gray-300 px-4 py-2">{{ $relacao->aluno->cpf_aluno }}
                                        </td>
                                        ...
                                        @if ($diploma->status < 3)
                                            ...
                                            <td class="border border-gray-300 px-4 py-2">
                                                <form method="POST"
                                                    action="{{ route('diplomas.remover', $relacao->id) }}"
    ...
    $(document).ready(function() {
        ...
        $('#adicionarAlunoForm').on('submit', function(event) {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error('Erro na requisição AJAX:', error);
                    console.error('Detalhes da resposta:', xhr
                    .responseText); // Exibe resposta detalhada
                    alert('Erro na requisição AJAX');
                }
            });
        });

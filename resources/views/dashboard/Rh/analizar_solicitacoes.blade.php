    ...
    <div class="bg-gray-200 p-8 rounded-md shadow-md w-full max-w-2xl"> <!-- Tornado responsivo com w-full -->
            ...
            <div class="mt-3 form-group flex flex-col">
                <form action="{{ route('dashboard.rh.aprovar_ticket', $ticket->id) }}" method="POST" style="display:inline;">
                    @csrf
                    <button type="submit" class="mb-2 btn btn-primary w-full border bg-green-500 hover:bg-red-700 text-white border-green-500 p-2 rounded-md btn btn-success">Aceitar</button>
                </form>
            </div> 
            <div class="mt-3 form-group flex flex-col"> 
                <form action="{{ route('dashboard.rh.rejeitar_ticket', $ticket->id) }}" method="POST" style="display:inline;">
                    @csrf
                    <button type="submit" class="btn btn-primary w-full border bg-red-500 hover:bg-red-700 text-white border-red-500 p-2 rounded-md btn btn-danger">Negar Pedido</button>
                </form>
            </div>
            </div>
        @else

...
<body class="font-inter bg-gray-100">
    ...
    <div class="flex flex-col items-center min-h-screen bg-gray-100 relative">
        ...
        <form method="GET" action="{{ route('dashboard.gestor.pedidos_ticket') }}" id="searchForm" class="mb-2">
            <input type="text"
                class="block w-1/1 p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                id="searchInput" name="search" placeholder="Buscar por funcionário"
                value="{{ request()->input('search') }}">
            <button type="submit"
                class="text-white ml-2 mr-10 mt-2 end-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Buscar</button>
            <button type="button"
                        ...
                        <tbody class="text-gray-700 text-sm">
                            @foreach ($tickets as $ticket)
                                <tr class="border-b">
                                    ...
                                    <td>
                                        <a href="{{ route('dashboard.gestor.detalhar_ticket', $ticket->id) }}"
                                            class="btn btn-info px-4 py-2">Ver detalhes</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>

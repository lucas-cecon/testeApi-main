            ...
            <form action="{{ route('dashboard.professor.store_ticket') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col md:col"> <!-- Ocupa 2 colunas em telas maiores -->
                        ...
                        <a href="{{ route('dashboard.gestor.pedidos_ticket') }}"><button type="button"
                    ...
                    @if (!in_array($ticket->status_ticket, [2, 3, 4]))
                        {{-- Verifica se o status é diferente de 3, 4 ou 5 --}}
                        <!-- Botões para aceitar ou negar o pedido -->

                        <div class="flex flex-col md:col">
                            <form action="{{ route('dashboard.gestor.aprovar_ticket', $ticket->id) }}" method="POST"
                                style="display:inline;">
                                @csrf
                                <button type="submit"
                                    class="btn btn-primary w-full border bg-green-500 hover:bg-green-700 text-white border-green-500 p-2 rounded-md">Aceitar
                                    Pedido</button>
                            </form>
                        </div>

                        <div class="flex flex-col md:col">
                            <form action="{{ route('dashboard.gestor.rejeitar_ticket', $ticket->id) }}" method="POST"
                                style="display:inline;">
                                @csrf
                                <button type="submit"
                                    class="btn btn-primary w-full border bg-red-500 hover:bg-red-700 text-white border-red-500 p-2 rounded-md">Negar
                                    Pedido</button>
                            </form>
                        @else

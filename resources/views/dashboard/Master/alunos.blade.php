        ...
        <div class="container mx-auto py-6 w-11/12">
            <div class="rounded-lg overflow-hidden">
                <div class="overflow-x-auto bg-gray-100"> <!-- Fundo alterado -->
                    ...
                    @else
                        <form method="GET" action="{{ route('dashboard.master.alunos.pesquisa') }}" id="filterForm"
                            ...
                            <input type="text" name="search"
                                ...
                                placeholder="Buscar por Nome do Aluno" value="{{ request('search') }}" />
                                    ...
                                    <table class="min-w-full table-auto border-collapse border border-gray-200">
                                        ...
                                        <tbody class="text-gray-700 text-sm">
                                            @foreach ($alunos as $aluno)
                                                <tr class="border-b">
                                                    <td class="border border-gray-300 px-2 py-2">{{ $aluno->id_aluno }}
                                                    </td>
                                                    <td class="border border-gray-300 px-2 py-2">{{ $aluno->nome }}
                                                    </td>
                                                    <td class="border border-gray-300 px-2 py-2">
                                                        {{ $aluno->cpf_aluno }}</td>
                                                    <td class="border border-gray-300 px-2 py-2">{{ $aluno->rg }}
                                                    </td>
                                                    <td class="border border-gray-300 px-2 py-2">
                                                        {{ $aluno->n_matricula }}</td>
                                                    <td class="border border-gray-300 px-2 py-2">
                                                        {{ $aluno->curso->curso ?? 'N/A' }}</td>
                                                    <td class="border border-gray-300 px-2 py-2">
                                                        {{ $aluno->statusAluno->status ?? 'N/A' }}</td>
                                                    <td class="border border-gray-300 px-2 py-2">
                                                        {{ $aluno->apmStatus->status ?? 'N/A' }}</td>
                                                    <td class="border border-gray-300 px-2 py-2">{{ $aluno->email }}
                                                    </td>
                                                    </td>
                                                    <td class="border border-gray-300 px-2 py-2">
                                                        <a href="{{ route('dashboard.master.alunos.editar', $aluno->id_aluno) }}"
                                                            class="btn text-blue-500 btn-primary text-decoration-line: underline">Editar</a>
                                                        <a href="{{ route('dashboard.master.alunos.detalhes', $aluno->id_aluno) }}"
                                                            class="btn text-blue-500 btn-secondary text-decoration-line: underline">Visualizar</a>
                                                        <form
                                                            action="{{ route('dashboard.master.alunos.deletar', $aluno->id_aluno) }}"
                                                            method="POST" style="display:inline;">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                                                class="btn text-red-500 btn-danger text-decoration-line: underline"

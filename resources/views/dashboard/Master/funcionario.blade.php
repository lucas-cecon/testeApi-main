...
<body class="font-inter bg-gray-100">
    ...
    <div class="flex flex-col items-center min-h-screen bg-gray-100 relative">
        ...
        <a href="{{ route('dashboard.master.cadastrar') }}"><button class="new-order-btn"> <svg
        ...
        <div class="container mx-auto py-6 w-11/12">
            <div class=" rounded-lg overflow-hidden">
                ...
                <!-- Campo de pesquisa -->
                <form method="GET" action="{{ route('dashboard.master.funcionarios.listar') }}" id="searchForm"
                    class="flex flex-col md:flex-row items-center justify-center mb-6">
                    <input type="text"
                        class="block w-1/3 p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mr-5"
                        id="searchInput" name="search" placeholder="Buscar por nome ou CPF"
                        value="{{ request()->input('search') }}">
                    <button type="submit"
                        class="text-white end-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 mr-5">Buscar</button>
                    <button type="button"
                                        ...
                                        @foreach ($funcionarios as $funcionario)
                                            <tr>
                                                <td class="border border-gray-300 px-4 py-2">
                                                    {{ $funcionario->ID_funcionario }}</td>
                                                <td class="border border-gray-300 px-4 py-2">{{ $funcionario->nome }}
                                                </td>
                                                <td class="border border-gray-300 px-4 py-2">{{ $funcionario->cpf }}
                                                </td>
                                                <td class="border border-gray-300 px-4 py-2">{{ $funcionario->nif }}
                                                </td>
                                                <td class="border border-gray-300 px-4 py-2">
                                                    {{ $funcionario->cargo->descricao ?? 'Cargo não encontrado' }}</td>
                                                <td class="border border-gray-300 px-4 py-2">
                                                    {{ $funcionario->horario->hora_inicio ?? 'N/A' }} -
                                                    {{ $funcionario->horario->hora_fim ?? 'N/A' }}</td>
                                                <td class="border border-gray-300 px-4 py-2">
                                                    <a class="text-blue-500 text-decoration-line: underline"
                                                        href="{{ route('dashboard.master.funcionarios.editar', ['id' => $funcionario->ID_funcionario]) }}">Editar</a>
                                                    <form
                                                        action="{{ route('dashboard.master.funcionarios.deletar', ['id' => $funcionario->ID_funcionario]) }}"
                                                        method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="text-red-500 text-decoration-line: underline"
                                                            onclick="return confirm('Tem certeza que deseja deletar este funcionário?')">Deletar</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach

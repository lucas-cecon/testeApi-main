    ...
    <div class="flex flex-col items-center mb-10 bg-gray-100">
        ...
        <div class="bg-gray-200 p-8 rounded-md shadow-md w-full max-w-2xl"> <!-- Tornado responsivo com w-full -->
            ...
            <form method="POST" action="{{ route('funcionarios.logout') }}">
                @csrf
                <button type="submit"
                    class="px-4 py-2 bg-red-600 text-white font-semibold rounded border border-red-700 hover:bg-red-700 mb-4">
                    Sair
                </button>
            </form>
            ...
            <div>
                @if ($funcionario->cargo == 1)
                    <a href="{{ route('dashboard.rh') }}"
                        class="inline-block px-4 py-2 bg-blue-600 text-white font-semibold rounded border border-blue-700 hover:bg-blue-700">Voltar
                        para o Dashboard RH</a>
                @elseif($funcionario->cargo == 2)
                    <a href="{{ route('dashboard.professor') }}"
                        class="inline-block px-4 py-2 bg-blue-600 text-white font-semibold rounded border border-blue-700 hover:bg-blue-700">Voltar
                        para o Dashboard Professor</a>
                @elseif($funcionario->cargo == 3)
                    <a href="{{ route('dashboard.gestor.index_arrumado') }}"
                        class="inline-block px-4 py-2 bg-blue-600 text-white font-semibold rounded border border-blue-700 hover:bg-blue-700">Voltar
                        para o Dashboard Gestor</a>
                @elseif($funcionario->cargo == 4)
                    <a href="{{ route('dashboard.master') }}"
                        class="inline-block px-4 py-2 bg-blue-600 text-white font-semibold rounded border border-blue-700 hover:bg-blue-700">Voltar
                        para o Dashboard Master</a>
                @endif
            </div>
        </div>
